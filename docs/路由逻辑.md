...
1. 当前采用“单向桥接”策略：只保留 `chat -> responses/messages`，不允许反向桥接。

2. 下游 `/v1/responses` 只允许走上游 `responses`。
   - 若渠道模型不支持 `responses`，直接拒绝（不再回落到 chat）。

3. 下游 `/v1/messages` 只允许走上游 `messages`。
   - 若渠道模型不支持 `messages`，直接拒绝（不再回落到 chat/responses）。

4. 下游 `/v1/chat/completions`：
   - 若渠道模型支持 `chat`，上游直连 `/v1/chat/completions`。
   - 若不支持 `chat` 但支持 `messages`，桥接到 `/v1/messages`。
   - 若不支持 `chat/messages` 但支持 `responses`，桥接到 `/v1/responses`。

5. 协议默认行为（无模型端点配置时）：
   - `openai` 系文本渠道默认走 `/v1/responses`；下游 `chat` 可桥接到 `responses`。
   - `anthropic/aws-claude` 默认走 `/v1/messages`；下游 `chat` 可桥接到 `messages`。
   - 对于下游 `messages` 请求，非 `messages` 能力渠道会被拒绝。

6. 候选渠道过滤仍有历史兼容判定，但最终以上游协商结果为准；
...
| --- | --- | --- | --- |
| `/v1/responses` | 渠道模型支持 `responses` | `/v1/responses` | 允许 |
| `/v1/responses` | 渠道模型不支持 `responses` | - | 拒绝 |
| `/v1/messages` | 渠道模型支持 `messages` | `/v1/messages` | 允许 |
| `/v1/messages` | 渠道模型不支持 `messages` | - | 拒绝 |
| `/v1/chat/completions` | 渠道模型支持 `chat` | `/v1/chat/completions` | 允许（直连） |
| `/v1/chat/completions` | 渠道模型不支持 `chat`，支持 `messages` | `/v1/messages` | 允许（chat -> messages） |
| `/v1/chat/completions` | 渠道模型不支持 `chat/messages`，支持 `responses` | `/v1/responses` | 允许（chat -> responses） |
| `/v1/chat/completions` | 渠道模型三者都不支持 | - | 拒绝 |
