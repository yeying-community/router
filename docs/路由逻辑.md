...
7. 如果 `system_settings` 中没有 `RetryTimes`，运行时默认值为 `0`，此时不会发生请求内切换。
8. 即使 `RetryTimes > 0`，当前实现也不保证一定切换成功，因为候选池可能被耗尽，或所有优先级层都已尝试失败。
...
| 类型 | 是否真换渠道 | 作用范围 | 是否影响当前请求 | 是否依赖 `RetryTimes` | 核心触发条件 |
| --- | --- | --- | --- | --- | --- |
| 新请求重新选路 | 是 | 请求之间 | 否 | 否 | 新请求进入 relay，且未指定渠道 |
| 请求级重试切换 | 是 | 同一个请求内部 | 是 | 是 | 首次上游失败，错误可重试，`RetryTimes > 0`，且仍有其他候选 |
...
1. `RetryTimes` 取自运行时配置。  
如果 `system_settings` 中没有 `RetryTimes`，默认值为 `0`。

2. 重试时会排除同一次请求里已经失败过的渠道。  
这意味着只要候选池里还有未失败的渠道，Router 不会再次打回同一条已失败渠道。

3. 重试时总是优先选择“当前仍未失败的最高优先级层”。  
...
- `system_settings` 里没有 `RetryTimes`
- 或者 `RetryTimes=0`
...
目标错误分类与处理策略，见 [上游错误处理策略](./上游错误处理策略.md)。
