...
3. 连续临时失败：
   - 单次 `GOAWAY`、`429`、`5xx`、`do_request_failed` 不会直接自动暂停能力
   - 同一 `channel + model + endpoint` 在 1 分钟内连续 5 次临时失败时，自动暂停该端点能力
   - 暂停后复用同一套自动测试恢复机制
...
| --- | --- | --- |
| 请求内切换 | 当前请求失败后立即重试 | 是 |
| 模型级能力自动暂停 | 当前请求结束后改变某个 `channel_model` 的可用性 | 否 |
| 端点级能力自动暂停 | 当前请求结束后改变某个 `channel + model + endpoint` 的可用性 | 否 |
| 自动禁用 / 账务禁用 | 当前请求结束后改变渠道状态 | 否 |
