...
- 识别 API token（支持原始 token key 与 `sk-` 前缀形式）。
- 不接受钱包 JWT 或 UCAN；这两类凭证只用于用户登录/身份类接口。
- 提取 `request_model`。
- 校验 token 的模型权限范围。
...
- 同一 `channel + model + endpoint` 在 1 分钟内连续 5 次临时上游失败

处理方式：

1. 模型级错误：
   - 将对应 `channel_models` 行记录运行时禁用原因
   - 上层只使用已发布的渠道模型；运行时禁用不再作为模型选择状态透出
   - 然后刷新该渠道关联的 `group_model_channels`

2. 端点级错误：
   - 将对应 `channel_model_endpoints` 行标记为 `enabled=false`
   - 不会直接暂停整个 `channel_model`
   - 后续同模型但其他端点仍可继续参与选路

3. 连续临时失败：
   - 单次 `GOAWAY`、`429`、`5xx`、`do_request_failed` 不会直接运行时禁用能力
   - 同一 `channel + model + endpoint` 在 1 分钟内连续 5 次临时失败时，运行时禁用该端点能力
   - 暂停后复用同一套自动测试恢复机制
...
明确的渠道级硬故障默认自动禁用，不再依赖全局开关。这里的目标是把已经不可用的上游渠道及时移出后续候选池，避免用户请求反复命中同一个故障渠道。
...
- `forbidden`
- 错误消息中包含 `credit balance is too low`、`api key expired`、`已欠费`、`余额不足`、`无可用资源包`、`账户已于`、`自动停用` 等
...
渠道级、模型级、端点级都有自动熔断和自动恢复链路，不需要管理员打开额外开关。历史上的 `AutomaticDisableChannelEnabled` 和 `AutomaticEnableChannelEnabled` 已废弃。

| 层级 | 熔断对象 | 什么时候熔断 | 怎么恢复 |
| --- | --- | --- | --- |
| 渠道级硬故障 | 整条渠道 | `401`、key 失效、账号停用、欠费、余额不足、无资源包、账号到期停用等明确不可用信号 | 余额/资源包类故障由独立恢复探测任务创建 `channel_recovery` 模型测试，测试成功后恢复渠道；其它硬故障需要管理员修正配置后再测试恢复 |
| 渠道级低成功率 | 整条渠道 | `metrics.enabled=true` 且最近窗口成功率低于 `metrics.success_rate_threshold` | 按 `metrics.auto_recover_after_seconds` 进入 `half_open`；配置无效时默认 300 秒。半开渠道低优先级参与真实请求，成功恢复，失败重新熔断 |
| 模型级能力 | `channel + model` | 上游明确返回模型不存在或模型无权限，例如 `model_not_found`、模型范围内的 `permission_error` | 立即创建恢复测试；后台每 10 分钟扫描 `disabled_by=runtime` 的模型能力并继续探测，测试成功后恢复该模型 |
| 端点级能力 | `channel + model + endpoint` | 上游明确返回端点不支持，或同一端点 1 分钟内连续 5 次临时失败 | 立即创建恢复测试；后台每 10 分钟扫描 `disabled_by=runtime` 的端点能力并继续探测，测试成功后恢复该端点 |

关键边界：

1. 请求内切换只影响当前请求；熔断和恢复主要影响后续请求。
2. 账务刷新不参与恢复判断，也不负责触发恢复测试；余额不足渠道是否恢复只看 `channel_recovery` 测试是否成功。
3. 模型级、端点级自动恢复只恢复运行态禁用的能力，管理员手动关闭的模型或端点不会被后台自动打开。
4. 恢复测试任务有去重机制，同一个 pending/running 测试不会无限重复堆积。
