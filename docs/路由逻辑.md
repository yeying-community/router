...
1. 请求进入 gin server
2. 全局 middleware（`TraceID`、`Language`、`Gin Logger`、`CORS`）
...
文本模型的端点协商规则固定为下面这组约束：

1. 如果上游模型配置的端点是 `/v1/responses`
   - 下游 `POST /v1/chat/completions`：Router 负责把 chat 请求转换为 responses 请求，再发到上游 `/v1/responses`
   - 下游 `POST /v1/responses`：Router 透传到上游 `/v1/responses`
2. 如果上游模型配置的端点不是 `/v1/responses`
   - 下游 `POST /v1/chat/completions`：按原端点发到上游 `/v1/chat/completions`
   - 下游 `POST /v1/responses`：直接拒绝，不能伪装成 responses 继续发给只支持 chat 的上游
3. 对于 `/v1/responses -> /v1/responses`
   - Router 不再用强类型结构体重建整份请求
   - 仅做最小规范化：`input` 统一保持 array 形态
   - 如果 `input` 是单个字符串，转换成标准数组，例如 `[{\"role\":\"user\",\"content\":\"...\"}]`
   - 其他未知字段保持原样透传，避免丢失 `instructions`、复杂 `tools` 等 responses 专属字段

后台模型测试也遵循相同原则：

1. 测试 `/v1/chat/completions` 时，构造 chat 请求体验证 chat 端点
2. 测试 `/v1/responses` 时，构造 responses 请求体验证 responses 端点
3. 不能用 chat 请求去“猜测” responses 是否可用，也不能用 responses 请求去“兼容”只支持 chat 的上游
