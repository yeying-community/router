...
1. 当前不再支持文本端点桥接；下游请求端点必须与渠道模型已启用端点一致。
...
5. 协议默认行为（无模型端点配置时）：
   - `openai` 系文本渠道默认走 `/v1/responses`。
   - `anthropic/aws-claude` 默认走 `/v1/messages`。
   - 默认值只用于初始配置；真实请求仍以渠道模型已启用端点为准。
...
| `/v1/chat/completions` | 渠道模型不支持 `chat` | - | 拒绝 |
