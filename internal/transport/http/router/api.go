...
func SetApiRouter(engine *gin.Engine) {
	...
	publicRouter.Use(middleware.GlobalAPIRateLimit())
	{
		...
		publicUserRoute := publicRouter.Group("/user")
		{
			...
			publicSelfRoute.Use(middleware.UserAuth())
			{
				...
				publicSelfRoute.GET("/package/subscription", user.GetCurrentUserActivePackageSubscription)
				...
				publicSelfRoute.GET("/topup/balance/lots", user.GetCurrentUserTopUpBalanceLots)
				publicSelfRoute.GET("/topup/balance/transactions", user.GetCurrentUserTopUpBalanceLotTransactions)
				...
				publicSelfRoute.POST("/topup/package/preview", user.PreviewPackagePurchase)
	...
	publicRelayRouter.Use(middleware.RelayLogger(), middleware.TokenAuth(), middleware.Distribute())
	{
		...
		publicRelayRouter.GET("/realtime", admin.Relay)
		publicRelayRouter.POST("/realtime/client_secrets", admin.Relay)
		publicRelayRouter.POST("/realtime/sessions", admin.Relay)
		publicRelayRouter.POST("/realtime/transcription_sessions", admin.Relay)
		publicRelayRouter.POST("/realtime/calls", admin.Relay)
		publicRelayRouter.POST("/videos", admin.Relay)
		publicRelayRouter.GET("/videos/:id", admin.Relay)
	...
	adminRouter.Use(middleware.GlobalAPIRateLimit())
	{
		...
		adminBillingRoute.Use(middleware.AdminAuth())
		{
			...
			adminBillingRoute.DELETE("/currencies/:code", middleware.RootAuth(), adminbilling.DeleteBillingCurrency)
		...
		adminChannelRoute.Use(middleware.AdminAuth())
		{
				...
				adminChannelRoute.GET("/alerts", channel.GetRecentChannelAlerts)
			...
			adminChannelRoute.GET("/circuit-breaker/events", channel.GetRecentChannelCircuitBreakerEvents)
			...
			adminChannelRoute.GET("/:id/billing", channel.GetChannelBilling)
			adminChannelRoute.GET("/:id/billing/profile", channel.GetChannelBillingProfile)
			adminChannelRoute.PUT("/:id/billing/profile", channel.UpdateChannelBillingProfile)
		...
		adminDashboardRoute := adminRouter.Group("/dashboard")
		adminDashboardRoute.Use(middleware.AdminAuth())
		{
			adminDashboardRoute.GET("/", dashboard.GetDashboard)
		}
		adminFlowRoute := adminRouter.Group("/flow")
		...
		adminGroupRoute.Use(middleware.AdminAuth())
		{
			...
			adminGroupRoute.GET("/:id/quota/daily", group.GetGroupDailyQuota)
			...
			adminGroupRoute.PUT("/:id/models", group.UpdateGroupModels)
			adminGroupRoute.PUT("/:id/models/:model", group.UpdateSingleGroupModel)
			adminGroupRoute.DELETE("/:id/models/:model", group.DeleteSingleGroupModel)
		}
